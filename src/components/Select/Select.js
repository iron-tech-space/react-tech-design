...
const Select = props => {
    ...
    const __setIsSelectAll = useCallback((val) => _setIsSelectAll(val), [_setIsSelectAll]);

    useEffect(() => {
        if (mode === "multiple")
            if (Array.isArray(value)) {
                if (_options.reduce((preValue, item) => value.includes(item.value) ? preValue + 1 : preValue, 0) === _options.length)
                    __setIsSelectAll(true);
                else
                    __setIsSelectAll(false);
            } else {
                if (!value) __setIsSelectAll(false);
            }
    }, [value, __setIsSelectAll, mode]);
    ...
    return (
        ...
        >
            {_options && _options.map(({ label, value, className, disabled }, i) =>
                <AntSelect.Option key={i.toString(36) + i} value={value} className={className}
                                  disabled={disabled}>{label}</AntSelect.Option>)}
            {tmpOption &&
                <AntSelect.Option key={generateUUID()} value={tmpOption.value} className={tmpOption.className}
                                  disabled={tmpOption.disabled}>{tmpOption.label}</AntSelect.Option>}
        </AntSelect>
    );
};
